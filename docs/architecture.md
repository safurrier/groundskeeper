...
review marker only from the GitHub actor authenticated for the current
Groundskeeper run. The review comment is written before the review label so an
interrupted transition remains recoverable.
